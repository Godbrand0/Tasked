# Tasked — The On-Chain Bounty Board for the Stacks Ecosystem
...
Tasked is a fully on-chain bounty protocol built on **Stacks** — a Bitcoin-secured smart contract layer — that connects task creators, contributors, and investors through trustless escrow, community-governed grant funding, and an **experience-based task matching system**.
...
| Dimension | EVM / Mezo version | Tasked / Stacks version |
|---|---|---|
...
│   │   │       └── Textarea.tsx
│   │   ├── hooks/
│   │   │   ├── useTasked.ts                    # All contract read/write hooks (Stacks.js)
...
│   ├── tests/
│   │   └── tasked_test.ts                      # Clarinet unit tests (Vitest)
│   ├── settings/
...
`contracts/contracts/tasked.clar` is the financial and governance backbone. It handles all USDX movements, grant voting, wave rewards, and experience gating without any off-chain intermediaries.
...
USDX is the native Bitcoin-backed stablecoin of the Stacks ecosystem. It is the primary unit of account for all financial operations in Tasked.
...
STX is the native token of the Stacks chain. Tasked gives STX holders a direct, productive role in the builder economy.
...
Developers who earn USDX on Tasked have a direct financial incentive to stay in the Stacks ecosystem. On-chain earnings create transaction history and economic identity that compounds with each completed task.

### Experience-Matched Quality

By gating applications on-chain by experience range, Tasked shifts the bounty market from "first to apply" to "best fit applies." Creators receive applications from developers with declared relevant experience; contributors spend time on work within their level rather than competing against specialists. Over time the experience distribution of completions acts as a real signal for skill development on the platform.

### Developer Identity Layer

GitHub-verified principals with declared experience tiers and completed task histories create a portable, on-chain reputation primitive. A contributor's Tasked history — their tier, USDX earned, and task count — is publicly queryable and can be used by other Stacks projects for hiring, access control, or on-chain credit scoring.
...
```bash
git clone https://github.com/<your-org>/tasked.git
cd tasked
pnpm install
```
