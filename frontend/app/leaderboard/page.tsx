...
import { MOCK_CREATOR_LEADERBOARD, MOCK_WAVE } from "@/lib/mock";
import { formatMUSD } from "@/lib/constants";

const MEDALS = ["🥇", "🥈", "🥉"];

export default function LeaderboardPage() {
  const sorted = [...MOCK_CREATOR_LEADERBOARD].sort((a, b) => b.selfFundedTasksPosted - a.selfFundedTasksPosted);
  const top3 = sorted.slice(0, 3);
  const rest = sorted.slice(3);

  function waveShare(tasksPosted: number) {
    return MOCK_WAVE.totalTasks > 0 ? (MOCK_WAVE.poolAmount * tasksPosted) / MOCK_WAVE.totalTasks : 0;
  }
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "48px 24px" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h1 style={{ fontSize: 36, fontWeight: 800, color: "#F0F0F5", margin: "0 0 12px", letterSpacing: "-0.02em" }}>Top Creators</h1>
          <p style={{ fontSize: 16, color: "#7070A0", margin: 0 }}>
            Ranked by self-funded tasks posted this wave (Wave #{MOCK_WAVE.waveId}). Grant-funded tasks don't count — only creators who commit their own MUSD earn wave rewards.
          </p>
        </div>
            ...
            return (
              <Link key={creator.address} href={`/profile/${creator.address}`} style={{ textDecoration: "none" }}>
                <div style={{ background: rank === 0 ? "linear-gradient(180deg, #F7931A18 0%, #111116 100%)" : "#111116", border: `1px solid ${rank === 0 ? "#F7931A40" : "#1E1E2A"}`, borderRadius: 16, padding: "24px 20px", textAlign: "center", height: heights[visualIdx], display: "flex", flexDirection: "column", justifyContent: "flex-end", transition: "transform 0.15s" }}
                  ...
                  <div style={{ fontSize: 14, fontWeight: 700, color: "#F0F0F5", marginBottom: 4 }}>{creator.username}</div>
                  <div style={{ fontSize: 18, fontWeight: 800, color: "#F7931A", marginTop: 10 }}>{creator.selfFundedTasksPosted}</div>
                  <div style={{ fontSize: 11, color: "#7070A0" }}>self-funded tasks</div>
                </div>
              </Link>
            );
          })}
        ...
        <div style={{ background: "#111116", border: "1px solid #1E1E2A", borderRadius: 16, overflow: "hidden" }}>
          {/* Table header */}
          <div style={{ display: "grid", gridTemplateColumns: "48px 1fr 120px 120px 80px", gap: 0, padding: "12px 24px", borderBottom: "1px solid #1E1E2A", fontSize: 11, fontWeight: 700, color: "#7070A0", letterSpacing: "0.06em", textTransform: "uppercase" }}>
            <span>#</span>
            <span>Creator</span>
          ...
          {sorted.map((creator, i) => (
            <Link key={creator.address} href={`/profile/${creator.address}`} style={{ textDecoration: "none" }}>
              <div style={{ display: "grid", gridTemplateColumns: "48px 1fr 120px 120px 80px", gap: 0, padding: "16px 24px", borderBottom: i < sorted.length - 1 ? "1px solid #1E1E2A" : "none", alignItems: "center", transition: "background 0.15s" }}
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600, color: "#F0F0F5" }}>{creator.username}</div>
                </div>
                <div style={{ textAlign: "center", fontSize: 15, fontWeight: 700, color: "#00D395" }}>{creator.selfFundedTasksPosted}</div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 15, fontWeight: 800, color: "#F7931A" }}>{formatMUSD(waveShare(creator.selfFundedTasksPosted))}</div>
                  <div style={{ fontSize: 11, color: "#7070A0" }}>MUSD</div>
                </div>
        ...
        <div style={{ textAlign: "center", marginTop: 24, fontSize: 12, color: "#7070A0" }}>
          Rankings reset every wave epoch (~30 days) · <Link href="/create" style={{ color: "#F7931A", textDecoration: "none" }}>Post a task →</Link>
        </div>
      </div>
    </div>
  );
