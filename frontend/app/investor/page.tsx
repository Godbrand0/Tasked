    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0F" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "40px 24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: "#7070A0" }}>
                Voting weight: <strong style={{ color: "#00D395" }}>{formatMUSD(votingWeight)} MUSD-eq</strong>
                {!hasStake && <span style={{ marginLeft: 8, color: "#F7931A", fontSize: 12 }}>— Stake MEZO to amplify 10×</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 40 }}>
          {[
            { label: "Total Deposited",    value: `${formatMUSD(PATRON.totalDeposited)} MUSD`, color: "#00D395", icon: "💰" },
            { label: "MEZO Staked",         value: `${formatMUSD(PATRON.mezoStaked)} MEZO`,       color: "#8B80FF", icon: "🔐" },
            { label: "Voting Weight",      value: `${formatMUSD(votingWeight)} eq`,             color: "#F7931A", icon: "⚖️" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 32, alignItems: "start" }}>
          ...
          <div>
                      ...
                      {vote && (
                        <div style={{ marginBottom: 16 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 8 }}>
                            <span style={{ color: "#00D395", fontWeight: 600 }}>For: {formatMUSD(vote.votesFor)} MUSD-eq</span>
                            <span style={{ color: "#F87171", fontWeight: 600 }}>Against: {formatMUSD(vote.votesAgainst)} MUSD-eq</span>
                          </div>
                        ...
                        <div>
                          ...
                          {!myVote && canVote && (
                            <div style={{ fontSize: 12, color: "#7070A0", textAlign: "center", marginTop: 8 }}>
                              Your weight: <strong style={{ color: "#F0F0F5" }}>{formatMUSD(votingWeight)} MUSD-eq</strong> · Deadline: {vote?.deadline ? new Date(vote.deadline * 1000).toLocaleDateString() : "—"}
                            </div>
                          )}
                        </div>
                      )}
            ...
            <div style={{ background: "#111116", border: "1px solid #1E1E2A", borderRadius: 14, padding: 24, marginTop: 24 }}>
              ...
              <code style={{ fontSize: 14, color: "#F0F0F5", fontFamily: "var(--font-geist-mono), monospace", display: "block", marginBottom: 16 }}>
                <span style={{ color: "#00D395" }}>musd-deposited</span>{" + ("}
                <span style={{ color: "#8B80FF" }}>mezo-staked</span>{" × "}
                <span style={{ color: "#F7931A" }}>10</span>{")"}
              </code>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <WeightRow label="MUSD deposited" value={`+${formatMUSD(PATRON.totalDeposited)}`} color="#00D395" />
                <WeightRow label="MEZO stake bonus (×10)" value={`+${formatMUSD(PATRON.mezoStaked * 10)}`} color="#8B80FF" />
                <div style={{ height: 1, background: "#1E1E2A" }} />
                <WeightRow label="Total weight" value={`${formatMUSD(votingWeight)} MUSD-eq`} color="#F7931A" bold />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {PATRON_TIERS.filter(t => t.id !== 99).map(t => {
                  ...
                  return (
                    <div key={t.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: 10, background: active ? t.bg : "transparent", border: `1px solid ${active ? t.color + "40" : "transparent"}` }}>
                      ...
                      <span style={{ fontSize: 12, color: active ? t.color : "#50507060" }}>{t.min.toLocaleString()} MUSD</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{ background: "#111116", border: "1px solid #1E1E2A", borderRadius: 14, padding: 24 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#00D395", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 12 }}>Deposit MUSD</div>
              <p style={{ fontSize: 12, color: "#7070A0", lineHeight: 1.6, marginBottom: 14 }}>
                Deposits are <strong style={{ color: "#F0F0F5" }}>permanent</strong> — this is ecosystem patronage, not a loan. Each MUSD = 1 governance unit.
              </p>
            ...
            {/* Stake MEZO */}
            <div style={{ background: "#111116", border: "1px solid #1E1E2A", borderRadius: 14, padding: 24 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#8B80FF", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 12 }}>Stake MEZO</div>
              <p style={{ fontSize: 12, color: "#7070A0", lineHeight: 1.6, marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <button disabled={PATRON.mezoStaked === 0}
                  style={{ flex: 1, background: "transparent", border: "1px solid #1E1E2A", color: "#9090B0", fontWeight: 700, fontSize: 13, padding: "10px", borderRadius: 10, cursor: PATRON.mezoStaked > 0 ? "pointer" : "not-allowed", opacity: PATRON.mezoStaked === 0 ? 0.5 : 1 }}>
                  Unstake
                </button>
              </div>
              <div style={{ fontSize: 12, color: "#7070A0", marginTop: 8 }}>
                Staked: <strong style={{ color: "#F0F0F5" }}>{formatMUSD(PATRON.mezoStaked)} MEZO</strong>
