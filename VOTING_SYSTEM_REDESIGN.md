...
**Sections 5, 5.8, and 7 are implemented, tested, security-reviewed (two rounds), and deployed to Mezo testnet.**

- **Currently deployed (2026-08-20, latest):** `0x3e72A1E45CD5c499f1fd48C8f102Bf6C28381F69` (Mezo testnet, chain 31611) — same as the previous deploy below, plus the Investor role removed entirely (see Section 10): `Role` is now just `{None, Creator, Contributor}`, `depositToPool` and `voteOnGrant` are open to any registered wallet, and `unstakeMezo`/`Patron.mezoStaked` were deleted as dead code. `veBTCEscrow` set to the verified testnet veBTC address; `veMEZOEscrow` unset; `approvedVoters` empty — **nobody can vote yet** until `setApprovedVoters` is called. `frontend/.env.local`'s `NEXT_PUBLIC_TASKIFY_CONTRACT` points at it.
- **Superseded, not migrated** (testnet only, no real funds at risk): `0x1FeBFE69533A2207f45f98b44Eb6564a122665b8` (pilot voter whitelist + veBTC redesign + Round 1/2 audit fixes, Investor role still gated support/vote — see below), `0x0ceC7A61B12d801a37143e6E223Cab907839cE3f` (Round 1 audit fixes only, pre-task-lifecycle-fixes), `0xD847FC1764D625f74498F28902b9801Ee9fDFDA0` (pilot whitelist, pre-audit-fixes), `0xce788B980a4339B8edCaaaFb42a9Bf2Ad8E0de6f` (veBTC redesign, no whitelist), `0x6DBca3d5bC3dE26741c80a5A284483BBb8EDACCb` (pre-redesign, MUSD/MEZO-divisor voting, 10 tasks). The `0x1FeBFE6…` build fixed all four Round 1 `TASKIFY_SECURITY_AUDIT.md` findings (escrow-address snapshotting, task-creation token allowlist, checks-effects-interactions reorder in `executeGrant`, ownership recovery + permissionless `advanceWave`) plus seven Round 2 findings from an external automated scan: `markExpired()` no longer touches `GrantPending` or `Submitted` tasks, a new creator-only `rejectSubmission()` for bad work (no refund — funds stay locked and the task reopens), a creator-chosen post-approval work duration (`applyForGrant`'s new `workDuration` param, 1–180 days, replaces a hardcoded 30 days), wave-credit reversal on `cancelTask`/`markExpired` within the same wave, an overflow-safe `Math.mulDiv` comparison in `executeGrant`, a zero-address guard on `setTreasuryAddress`, and a new permissionless `claimStrandedWaveFunds()` sweeping grant-only wave fees to the treasury.
- **See `TASKIFY_SECURITY_AUDIT.md`** for the full two-round security review — Round 1: one High, one Medium, two Low findings; Round 2: six fixed Highs/Mediums plus one (F-233677) verified real and left as an accepted manual-retry mitigation. All fixed findings are covered by regression tests. Not a substitute for a professional third-party audit, which is still needed before real capital is at stake.
...
- `contracts/src/Taskify.sol` — `IMezoVotingEscrow`, `veBTCEscrow`/`veMEZOEscrow` + setters, `_votingWeight`/`getVotingWeight`/`getProposalVotingWeight`, `GrantVote.snapshotTimestamp` + `escrowAtSnapshot` (both locked per-proposal) wired through `applyForGrant`/`voteOnGrant`, `stakeMezo` removed, `unstakeMezo`/`Patron.mezoStaked` later deleted too once nothing could ever set the field (Section 10), dead divisor constants removed, `approvedVoters`/`setApprovedVoters` pilot whitelist (Section 5.8), `transferOwnership`, permissionless `advanceWave`, token allowlist on task creation, creator-chosen `workDuration` on grant applications, `rejectSubmission`, `claimStrandedWaveFunds`, wave-credit reversal helper.
- `contracts/src/MockVotingEscrow.sol` — checkpoint-aware test double.
- `contracts/test/Taskify.t.sol` + `contracts/test/SecurityAudit.t.sol` + `contracts/test/TaskLifecycleAudit.t.sol` — 26 passing tests total, including snapshot-vs-live-weight, the NFT cap, the whitelist gate, six Round 1 audit regression tests, and seven Round 2 audit regression tests.
...
- `frontend/lib/taskify.ts`, `frontend/lib/use-taskify.ts`, `frontend/app/investor/page.tsx` (later split into `frontend/app/support/page.tsx` + `frontend/app/vote/page.tsx` — see the end-of-file update note) — old divisor-based `votingWeight()` removed, per-proposal weight now read via `getProposalVotingWeight` (not the live-escrow `getVotingWeight`, which is preview-only) so the UI always matches what a vote will actually record; `useIsApprovedVoter` gates the vote UI alongside weight; Stake MEZO UI replaced with a migration-only legacy-withdrawal panel; Mezo Earn CTA added.
- `frontend/app/create/page.tsx` — grant applications now include a "Time to Complete (if approved)" selector (7/14/30/60/90 days), passed through as `applyForGrant`'s `workDuration` argument.
- `frontend/app/tasks/[id]/page.tsx` — creators can now reject a bad submission (`rejectSubmission`) as an alternative to approving it, reopening the task without refunding escrow.
...
**Decided (superseded — see end-of-file update note):** keep the `Role.Investor` gate at line 470 (`if (users[msg.sender].role != Role.Investor) revert NoStake();`). It's a free `registerUser()` call, not comparable friction to the staking pool being removed, and it keeps this module consistent with every other role in the contract (Creator, Contributor also require registration). It also gives Taskify identity for display purposes (username, tier badge) alongside the externally-sourced weight. No code change needed here beyond the weight computation above.
...
`voteOnGrant()` gains one more check, right after the existing role check: `if (!approvedVoters[msg.sender]) revert NotApprovedVoter();`. `getVotingWeight()` is deliberately untouched — it stays a pure read of external veBTC weight — so the frontend needs both `getVotingWeight > 0` **and** `approvedVoters[address]` to correctly show whether a wallet can actually vote (see `useIsApprovedVoter` in `frontend/lib/use-taskify.ts`, and `canVote`/`canVoteThisTask` in `frontend/app/vote/page.tsx`).
...
`frontend/app/investor/page.tsx` (later split into `frontend/app/support/page.tsx` + `frontend/app/vote/page.tsx` — see the end-of-file update note)
...
- Dual gate (`Role.Investor` + veBTC weight) — kept both at the time (5.4); the `Role.Investor` half of this gate was later removed entirely along with the Investor role itself — see the end-of-file update note.
...
## 10. Update (2026-08-20): Investor role removed

DevRel feedback: creators want to be able to support the grant pool too, not just a separate "investor" class. That, plus wanting to formally decouple "who can support the pool" from "who can vote," made the `Role.Investor`-specific gates kept in Sections 5.4/9 above obsolete. Changes:

- `Role` enum is now just `{None, Creator, Contributor}` — Investor removed. `registerUser` only accepts Creator or Contributor.
- `depositToPool` — the `Role.Investor` check was replaced with a plain "is registered" check. Any registered wallet, Creator or Contributor, can support the grant pool.
- `voteOnGrant` — the `Role.Investor` check was removed outright, not replaced with a weaker role check. The sole gates are now `approvedVoters[msg.sender]` (pilot whitelist) and real veBTC weight — fully independent of role, and independent of whether the wallet has ever deposited into the pool.
- `unstakeMezo` and `Patron.mezoStaked` were deleted entirely (not just re-gated). Both were already migration-only dead code — `stakeMezo()` had been removed earlier, so nothing could ever set `mezoStaked` above zero on a fresh deploy — and removing the role they were gated on left no remaining legitimate caller.
- `registerUser` now unconditionally initializes `patrons[msg.sender]` (previously only for `Role.Investor`), so `tier` correctly reads "None" (99) rather than colliding with Solidity's zero-value default for any Creator/Contributor who hasn't deposited yet.
- Frontend: `frontend/app/investor/page.tsx` was deleted and split into `frontend/app/support/page.tsx` (deposit UI, patron tier) and `frontend/app/vote/page.tsx` (grant list, voting UI) — both gated on "is registered," not on role.
- `supabase/schema.sql`'s `user_role` enum dropped `'investor'`; `patrons_onchain.mezo_staked` column dropped — see `supabase/migrations/0014_remove_investor_role.sql`.

Contract redeployed to Mezo testnet as part of this change — see the top-level README for the current address.
