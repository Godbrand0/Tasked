...
**Trust boundary:** requires `CONTRACT_OWNER` access — either genuine malicious intent, or (more realistically, and the more common real-world case) a compromised deployer private key. `CONTRACT_OWNER` was a single EOA when this review was written (see L-2), which made key compromise the more likely real-world trigger for this than owner malice. Since 2026-09-16 the mainnet owner is a 2-of-3 Safe (`0xcfeC02DfC63FcA293b5F9c2856bb1370965D2a31`), so a single compromised key is no longer sufficient — see the Governance & Ownership section of the root `README.md`.
...
**✅ Fixed.** `CONTRACT_OWNER` is no longer `immutable`; a new `transferOwnership(address newOwner)` (owner-only, rejects the zero address via a new `InvalidOwner` error) allows rotating to a multisig or recovering from a compromised key without a redeploy (`test_OwnershipCanBeTransferred`, `test_TransferOwnershipRejectsZeroAddress`). `advanceWave` is now fully permissionless — gated only by the epoch having genuinely elapsed, exactly as recommended (`test_AdvanceWaveIsPermissionless`). Moving the actual deployed owner to a real multisig was left as an operational step; **it has since been taken.** On 2026-09-16 mainnet `CONTRACT_OWNER` and `treasuryAddress` were both transferred to the Safe `0xcfeC02DfC63FcA293b5F9c2856bb1370965D2a31`, later expanded to 2-of-3 with an independent Mezo g6 community signer. See Governance & Ownership in the root `README.md`. Ownership transfer is now two-step (`transferOwnership` then `acceptOwnership`). The remaining gap is the absence of a timelock on upgrades.
